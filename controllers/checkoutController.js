...
const placeOrder = async (req, res) => {
    try {
        ...
        if (cartData.length > 0) {
            const cartTotal = cartData[0];

            if (cartTotal && cartTotal.products) {
                const productsArray = cartTotal.products.map(product => ({
                    product: product.product._id,
                    price: product.product.price,
                    quantity: product.quantity,
                }));

                totalPrice = productsArray.reduce((sum, product) => sum + product.price * product.quantity, 0);
            } else {
                console.error('Error loading cart: Cart or products property is null or undefined.');
                res.redirect('/allProduct'); // or handle the error appropriately
                return;
            }
        } else {
            console.error('Error loading cart: CartData array is empty.');
            res.redirect('/allProduct'); // or handle the error appropriately
            return;
        }
        ...
        if (payment === 'COD') {
            ...
            if (!cartData) {
                const response = { errorMessages: 'Cart not found for the user.' };
                return res.status(400).json(response);
            }
        
            const cartTotal = cartData.products || [];
        
            const totalPrice = cartTotal.length > 0
                ? cartTotal.reduce((sum, product) => sum + product.product.price * product.quantity, 0)
                : 0;
        
            const wallet = await Wallet.findOne({ userId: id });
        
            if (!wallet) {
                const response = { errorMessages: 'Wallet not found for the user.' };
                return res.status(400).json(response);
            }
            ...
            if (totalPrice <= wallet.totalPrice) {
                const updatedTotal = wallet.totalPrice - totalPrice;
                console.log('Updated Wallet Total Price:', updatedTotal);
                ...
                console.log('Updated Wallet:', wallet1);
